function[Energy_Norm,Energy_NormhH,Rel_Energy_Norm,Rel_Energy_NormhH,CNAms,CNABc]=LODsolf(NR,C11,C22,Nx,Ny)
...
%% Reference Solution
%L2 = ichol(AB);vr=pcg(AB,FL2,1e-6,100,L2,L2');
...
%% Relative error of Coarse Regular FEM solution with repect to LOD solution
w=v'*Mlod;
vLR=vc'*Ph-w;
RelErrorCRFvsLOD=sqrt(vLR*AB*vLR')/sqrt(w*AB*w');

%% Condition Number of Ams
CNAms=cond(full(Ams));
%% Condition Number of ABc
CNABc=cond(full(ABc));
end