...
function [Ind,c]=Projcoeffs2(I,J,a,Aint,Aside,h,H,i,j,Nx,Ny,type_sq,sqnum)

ILx1Nxi1l = (h*(h - 3*h*j + 3*H*(1 + J)))/(6*H);
ILx2Nxi1l = (h*(h*(-1 + 3*j) - 3*H*J))/(6*H);
ILx1Nxi2r = -((h*(h + 3*h*j - 3*H*(1 + J)))/(6*H));
ILx2Nxi2r = (h*(h + 3*h*j - 3*H*J))/(6*H);
ILy1Nyi1l = (h*(h - 3*h*i + 3*H*(1 + I)))/(6*H);
ILy2Nyi1l = (h*(h*(-1 + 3*i) - 3*H*I))/(6*H);
ILy1Nyi2r = -((h*(h + 3*h*i - 3*H*(1 + I)))/(6*H));
ILy2Nyi2r = (h*(h + 3*h*i - 3*H*I))/(6*H);
